...
class PrenatalChildAdptr(val con: Context, val lst: ArrayList<ProgramChaptersDataItem?>) :
    RecyclerView.Adapter<PrenatalChildAdptr.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.mView.textView36.text =
            (if (lst[position]?.duration?.isNotEmpty() == true) lst[position]?.duration?.toInt() else 0)?.let {
