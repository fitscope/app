...
class RecumentCategoriesAptr(val con: Context, val lst: ArrayList<ProgramChaptersDataItem?>) :
    RecyclerView.Adapter<RecumentCategoriesAptr.ViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): RecumentCategoriesAptr.ViewHolder {
        return ViewHolder(
    ...
    override fun onBindViewHolder(holder: RecumentCategoriesAptr.ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (PrefUtils.with(con).getString(Enums.isLogin.toString(), "false") == "true") {
                con.startActivity(
                    ...
                    ).putExtra(Constants.From, Enums.FromRecumbentSingleViewResults.toString())
