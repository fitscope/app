...
class ClassHomeViewModel(private val repo: ClassRepo) : ViewModel() {
    private val _classHomeData = MutableLiveData<NetworkReponse<ClassHomeRes>>()
    val classHomeData: LiveData<NetworkReponse<ClassHomeRes>> get() = _classHomeData
    ...
    fun getClassHomeData() {
        viewModelScope.launch {
            try {
                _classHomeData.value = NetworkReponse.Success(repo.getClassHomeData().body())
            } catch (e: Exception) {
                _classHomeData.value = NetworkReponse.Error(e.message ?: "")
            }
